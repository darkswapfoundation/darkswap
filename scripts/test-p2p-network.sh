...
check_dependencies() {
    ...
    if [ ! -f "./target/debug/darkswap-cli" ]; then
        print_error "darkswap-cli binary not found at ./target/debug/darkswap-cli"
        missing_deps=true
    else
        print_success "darkswap-cli binary found at ./target/debug/darkswap-cli"
    fi
...
test_peer_discovery() {
    ...
    # Test if ./target/debug/darkswap-cli p2p discover command exists
    if ! ./target/debug/darkswap-cli p2p discover --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p discover command not available"
        return
    fi
    
    # Test DHT-based discovery
    print_info "Testing DHT-based discovery..."
    local dht_result=$(./target/debug/darkswap-cli p2p discover --method dht --json 2>/dev/null || echo '{"peers": []}')
    ...
    if [ "$RELAY_TEST" = true ]; then
        ...
        local relay_result=$(./target/debug/darkswap-cli p2p discover --method relay --json 2>/dev/null || echo '{"peers": []}')
    ...
    local local_result=$(./target/debug/darkswap-cli p2p discover --method local --json 2>/dev/null || echo '{"peers": []}')
    ...
    local combined_result=$(./target/debug/darkswap-cli p2p discover --json 2>/dev/null || echo '{"peers": []}')
...
test_direct_connections() {
    ...
    # Test if ./target/debug/darkswap-cli p2p connect command exists
    if ! ./target/debug/darkswap-cli p2p connect --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p connect command not available"
        return
    fi
    
    # Get a list of peers
    local peers_result=$(./target/debug/darkswap-cli p2p discover --json 2>/dev/null || echo '{"peers": []}')
    ...
    for peer in $peers; do
        ...
        local result=$(./target/debug/darkswap-cli p2p connect --peer-id "$peer" --timeout "$TEST_TIMEOUT" --json 2>/dev/null || echo '{"success": false}')
        ...
        if [ "$success" = "true" ] && [ "$connection_type" = "direct" ]; then
            ...
            local send_result=$(./target/debug/darkswap-cli p2p send --peer-id "$peer" --message "$message" --json 2>/dev/null || echo '{"success": false}')
            ...
            ./target/debug/darkswap-cli p2p disconnect --peer-id "$peer" > /dev/null 2>&1
        else
...
test_relay_connections() {
    ...
    for peer in $peers; do
        ...
        local result=$(./target/debug/darkswap-cli p2p connect --peer-id "$peer" --force-relay --timeout "$TEST_TIMEOUT" --json 2>/dev/null || echo '{"success": false}')
        local success=$(echo "$result" | jq -r '.success')
        local connection_type=$(echo "$result" | jq -r '.connectionType // "unknown"')
...
test_webrtc_signaling() {
    ...
    # Test if ./target/debug/darkswap-cli p2p test-webrtc command exists
    if ! ./target/debug/darkswap-cli p2p test-webrtc --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p test-webrtc command not available"
        return
    fi
    
    # Check if signaling server is reachable
    print_info "Testing signaling server reachability..."
    local signaling_url=$(./target/debug/darkswap-cli config get p2p.signaling_url 2>/dev/null || echo "unknown")
    ...
    local webrtc_test=$(./target/debug/darkswap-cli p2p test-webrtc --timeout "$TEST_TIMEOUT" --json 2>/dev/null || echo '{"success": false}')
...
test_message_broadcasting() {
    ...
    # Test if ./target/debug/darkswap-cli p2p broadcast command exists
    if ! ./target/debug/darkswap-cli p2p broadcast --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p broadcast command not available"
        return
    fi
    ...
    local broadcast_result=$(./target/debug/darkswap-cli p2p broadcast --message "$message" --json 2>/dev/null || echo '{"success": false, "recipientCount": 0}')
...
test_stress_conditions() {
    ...
    # Test if ./target/debug/darkswap-cli p2p connect and send commands exist
    if ! ./target/debug/darkswap-cli p2p connect --help > /dev/null 2>&1 || ! ./target/debug/darkswap-cli p2p send --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p connect or send commands not available"
        return
    fi
    ...
    local connect_result=$(./target/debug/darkswap-cli p2p connect --peer-id "$peer" --timeout "$TEST_TIMEOUT" --json 2>/dev/null || echo '{"success": false}')
...
test_connection_recovery() {
    ...
    # Test if ./target/debug/darkswap-cli p2p restart command exists
    if ! ./target/debug/darkswap-cli p2p restart --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p restart command not available"
        return
    fi
    ...
    ./target/debug/darkswap-cli p2p restart > /dev/null 2>&1
    sleep 5
    
    # Check if the connection was recovered
    print_info "Checking if connection was recovered..."
    local status_result=$(./target/debug/darkswap-cli p2p status --json 2>/dev/null || echo '{"connections": []}')
...
test_network_metrics() {
    ...
    # Test if ./target/debug/darkswap-cli p2p metrics command exists
    if ! ./target/debug/darkswap-cli p2p metrics --help > /dev/null 2>&1; then
        print_error "./target/debug/darkswap-cli p2p metrics command not available"
        return
    fi
    
    # Get network metrics
    print_info "Getting network metrics..."
    local metrics=$(./target/debug/darkswap-cli p2p metrics --json 2>/dev/null || echo '{"peerCount": 0, "connectedPeers": 0, "messagesSent": 0, "messagesReceived": 0, "directConnections": 0, "relayConnections": 0, "averageLatency": 0}')
...
generate_report() {
    ...
    local darkswap_version=$(./target/debug/darkswap-cli --version 2>/dev/null | head -n 1 || echo "unknown")
    local network_info=$(./target/debug/darkswap-cli config get network 2>/dev/null || echo '"unknown"')
    
    # Collect P2P metrics
    local p2p_metrics=$(./target/debug/darkswap-cli p2p metrics --json 2>/dev/null || echo '{}')
...
EOF
    ...
    if ./target/debug/darkswap-cli p2p discover --help > /dev/null 2>&1; then
        cat >> "$report_file" << EOF
    "peerDiscovery": {
      "dhtDiscovery": $(./target/debug/darkswap-cli p2p discover --method dht --json 2>/dev/null || echo '{"peers": []}'),
      "relayDiscovery": $(./target/debug/darkswap-cli p2p discover --method relay --json 2>/dev/null || echo '{"peers": []}'),
      "localDiscovery": $(./target/debug/darkswap-cli p2p discover --method local --json 2>/dev/null || echo '{"peers": []}')
    },
EOF
    fi

    # Add connection status if available
    if ./target/debug/darkswap-cli p2p status --help > /dev/null 2>&1; then
        cat >> "$report_file" << EOF
    "connectionStatus": $(./target/debug/darkswap-cli p2p status --json 2>/dev/null || echo '{}'),
EOF
    fi

    # Add WebRTC test results if available
    if [ "$SIGNALING_TEST" = true ] && ./target/debug/darkswap-cli p2p test-webrtc --help > /dev/null 2>&1; then
        cat >> "$report_file" << EOF
    "webrtcTest": $(./target/debug/darkswap-cli p2p test-webrtc --json 2>/dev/null || echo '{}')
EOF
    else
