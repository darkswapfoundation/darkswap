...
import { WalletConnectProvider as WalletConnectProviderClass, createWalletConnectProvider } from '../utils/WalletConnectProvider';
import { ErrorCode, DarkSwapError } from '../utils/ErrorHandling';

/**
 * WalletConnect context state
 */
interface WalletConnectContextState {
  provider: WalletConnectProviderClass;
  isSupported: boolean;
  isConnected: boolean;
  address: string | null;
  chainId: number | null;
 ...
 */
const WalletConnectContext = createContext<WalletConnectContextState | null>(null);
 ...
 */
export const WalletConnectProvider: React.FC<WalletConnectProviderProps> = ({ children, options }) => {
  // Create WalletConnect provider
  const [provider] = useState<WalletConnectProviderClass>(() => createWalletConnectProvider(options));
  ...
  useEffect(() => {
    ...
    const isWalletConnectSupported = provider.isWalletConnectSupported();
    setIsSupported(isWalletConnectSupported);
    
    // If WalletConnect is supported, check if it's connected
  ...
  // Context value
  const value: WalletConnectContextState = {
    provider,
    isSupported,
    isConnected,
    address,
