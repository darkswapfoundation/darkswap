/**
 * Unit tests for SimdDarkSwapDemo
 */

import React from 'react';
import { render, screen, fireEvent, waitFor } from '@testing-library/react';
import '@testing-library/jest-dom';
import SimdDarkSwapDemo from '../SimdDarkSwapDemo';
...
// Mock the SimdDarkSwapClient
jest.mock('../../utils/SimdDarkSwapClient');

describe('SimdDarkSwapDemo', () => {
  // Mock implementation of SimdDarkSwapClient
  const mockSimdDarkSwapClient = {
  ...
  beforeEach(() => {
    ...
    // Mock the SimdDarkSwapClient constructor
    (SimdDarkSwapClient as jest.Mock).mockImplementation(() => mockSimdDarkSwapClient);
  ...
  it('should render the component', () => {
    render(<SimdDarkSwapDemo />);
    expect(screen.getByText('SIMD-enabled DarkSwap Demo')).toBeInTheDocument();
  });

  it('should initialize the client on mount', async () => {
    render(<SimdDarkSwapDemo />);
    await waitFor(() => {
      expect(mockSimdDarkSwapClient.initialize).toHaveBeenCalled();
    });
  });

  it('should display SIMD support status', async () => {
    render(<SimdDarkSwapDemo />);
    ...
    // Check that the SIMD support status is displayed
    expect(screen.getByText(/SIMD instructions are supported in your browser./)).toBeInTheDocument();
  });

  it('should display SIMD not supported status', async () => {
    // Mock isSimdSupported to return false
    mockSimdDarkSwapClient.isSimdSupported.mockReturnValue(false);
    ...
    // Check that the SIMD not supported status is displayed
    expect(screen.getByText(/SIMD instructions are not supported in your browser./)).toBeInTheDocument();
    expect(screen.getByText(/Falling back to regular WebAssembly./)).toBeInTheDocument();
  });
  ...
  it('should create a DarkSwap instance when the Create Instance button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockSimdDarkSwapClient.create).toHaveBeenCalled();
    });
  });
  ...
  it('should start DarkSwap when the Start DarkSwap button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockSimdDarkSwapClient.start).toHaveBeenCalled();
    });
  });
  ...
  it('should get wallet info when the Get Wallet Info button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockSimdDarkSwapClient.getAddress).toHaveBeenCalled();
      expect(mockSimdDarkSwapClient.getBalance).toHaveBeenCalled();
  ...
  it('should get market data when the Get Market Data button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(mockSimdDarkSwapClient.getOrders).toHaveBeenCalled();
      expect(mockSimdDarkSwapClient.getBestBidAsk).toHaveBeenCalled();
