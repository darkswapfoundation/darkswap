...
/**
 * Dynamic Chunk Size DarkSwap demo component
 */
const DynamicChunkSizeDarkSwapDemo: React.FC = () => {
  // DarkSwap client
  const [client, setClient] = useState<DynamicChunkSizeDarkSwapClient | null>(null);
  // Loading state
  ...
  useEffect(() => {
    const initClient = async () => {
      try {
        ...
        const darkswapClient = new DynamicChunkSizeDarkSwapClient('/darkswap-wasm/darkswap_wasm_bg.wasm');
        ...
        // Get network speed
        setNetworkSpeed(darkswapClient.getNetworkSpeed());
  ...
  return (
    <div className="darkswap-demo">
      ...
      <div className="streaming-status">
        ...
        {networkSpeed ? (
          <div>
            <p>Download Speed: {formatBytes(networkSpeed.downloadSpeed)}/s</p>
            <p>Round-Trip Time: {networkSpeed.rtt.toFixed(2)} ms</p>
            <p>Last Measured: {new Date(networkSpeed.timestamp).toLocaleString()}</p>
          </div>
        ) : (
          <p>Network speed has not been measured yet.</p>
        )}
      </div>
...
export default DynamicChunkSizeDarkSwapDemo;