...
/**
 * Combined Optimizations DarkSwap demo component
 */
const CombinedOptimizationsDarkSwapDemo: React.FC = () => {
  // DarkSwap client
  const [client, setClient] = useState<CombinedOptimizationsDarkSwapClient | null>(null);
  // Loading state
  ...
  // Optimizations used
  const [optimizationsUsed, setOptimizationsUsed] = useState<{
    webWorker: boolean;
    streaming: boolean;
    simd: boolean;
  ...
  useEffect(() => {
    const initClient = async () => {
      try {
        ...
        const darkswapClient = new CombinedOptimizationsDarkSwapClient(
        ...
        // Get optimizations used
        setOptimizationsUsed(darkswapClient.getOptimizationsUsed());
  ...
  return (
    <div className="darkswap-demo">
      ...
      <div className="streaming-status">
        <h2>Optimizations Status</h2>
        {optimizationsUsed ? (
          <div>
            <p>
              Web Worker: <span className={optimizationsUsed.webWorker ? 'enabled' : 'disabled'}>
                {optimizationsUsed.webWorker ? 'Enabled' : 'Disabled'}
              </span>
            </p>
            <p>
              Streaming Compilation: <span className={optimizationsUsed.streaming ? 'enabled' : 'disabled'}>
                {optimizationsUsed.streaming ? 'Enabled' : 'Disabled'}
              </span>
            </p>
            <p>
              SIMD Instructions: <span className={optimizationsUsed.simd ? 'enabled' : 'disabled'}>
                {optimizationsUsed.simd ? 'Enabled' : 'Disabled'}
              </span>
            </p>
            <p>
              Shared Memory: <span className={optimizationsUsed.sharedMemory ? 'enabled' : 'disabled'}>
                {optimizationsUsed.sharedMemory ? 'Enabled' : 'Disabled'}
              </span>
            </p>
          </div>
        ) : (
          <p>Optimizations have not been detected yet.</p>
        )}
      </div>
...
export default CombinedOptimizationsDarkSwapDemo;