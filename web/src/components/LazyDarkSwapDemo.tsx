...
/**
 * Lazy-loaded DarkSwap demo component
 */
const LazyDarkSwapDemo: React.FC = () => {
  // DarkSwap client
  const [client, setClient] = useState<LazyDarkSwapClient | null>(null);
  // Loading state
  ...
  useEffect(() => {
    const initClient = async () => {
      try {
        ...
        const darkswapClient = new LazyDarkSwapClient();
  ...
  const loadClient = async () => {
    ...
    try {
      ...
      // Create DarkSwap instance (this will load the WebAssembly module)
      await client.create({
      ...
      console.error('Failed to load DarkSwap client:', err);
      setError(`Failed to load DarkSwap client: ${err}`);
      setLoading(false);
    }
  };
  ...
  return (
    <div className="darkswap-demo">
      ...
      <div className="performance-metrics">
        ...
        <p>Loading time: {metrics.loadTime.toFixed(2)} ms</p>
      ...
      <div className="client-actions">
        ...
        <button onClick={loadClient} disabled={!!address}>Load Client</button>
        <button onClick={getWalletInfo} disabled={!address && !client}>Get Wallet Info</button>
        <button onClick={getMarketData} disabled={!address && !client}>Get Market Data</button>
      </div>
...
export default LazyDarkSwapDemo;