...
import { StreamingDarkSwapClient } from '../utils/StreamingDarkSwapClient';
import { AssetType, OrderSide, BitcoinNetwork } from '../utils/DarkSwapClient';
import '../styles/DarkSwapDemo.css';

/**
 * Streaming DarkSwap demo component
 */
const StreamingDarkSwapDemo: React.FC = () => {
  // DarkSwap client
  const [client, setClient] = useState<StreamingDarkSwapClient | null>(null);
  // Loading state
  ...
  useEffect(() => {
    const initClient = async () => {
      try {
        ...
        const isStreamingSupported = typeof WebAssembly.compileStreaming === 'function';
        setStreamingSupported(isStreamingSupported);

        const startTime = performance.now();

        // Create DarkSwap client
        const darkswapClient = new StreamingDarkSwapClient();
  ...
  return (
    <div className="darkswap-demo">
      ...
      <div className="streaming-status">
        ...
        <p>
          Streaming compilation is {streamingSupported ? 'supported' : 'not supported'} in your browser.
          {!streamingSupported && ' Falling back to regular compilation.'}
        </p>
      </div>
...
export default StreamingDarkSwapDemo;