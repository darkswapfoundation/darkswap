...
/**
 * Shared Memory DarkSwap demo component
 */
const SharedMemoryDarkSwapDemo: React.FC = () => {
  // DarkSwap client
  const [client, setClient] = useState<SharedMemoryDarkSwapClient | null>(null);
  // Loading state
  ...
  const [sharedMemorySupported, setSharedMemorySupported] = useState<boolean | null>(null);
  // Memory options
  ...
  useEffect(() => {
    const initClient = async () => {
      try {
        ...
        const darkswapClient = new SharedMemoryDarkSwapClient('/darkswap-wasm/darkswap_wasm_bg.wasm', {
        ...
        setSharedMemorySupported(darkswapClient.isSharedMemorySupported());
  ...
  /**
   * Handle memory options change
   */
  const handleMemoryOptionsChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value } = e.target;
    const numValue = parseInt(value, 10);
    if (!isNaN(numValue) && numValue > 0) {
      setMemoryOptions((prevOptions) => ({
        ...prevOptions,
        [name]: numValue,
      }));
    }
  ...
  return (
    <div className="darkswap-demo">
      ...
      <div className="streaming-status">
        <h2>Shared Memory Status</h2>
        <p>
          Shared memory is {sharedMemorySupported ? 'supported' : 'not supported'} in your browser.
          {!sharedMemorySupported && ' Falling back to regular WebAssembly.'}
        </p>
      </div>
      
      <div className="code-split-config">
        <h2>Memory Configuration</h2>
        <div className="config-form">
          <div>
            <label htmlFor="initial">Initial Memory (pages):</label>
            <input
              type="number"
              id="initial"
              name="initial"
              value={memoryOptions.initial}
              onChange={handleMemoryOptionsChange}
              min="1"
            />
          </div>
          <div>
            <label htmlFor="maximum">Maximum Memory (pages):</label>
            <input
              type="number"
              id="maximum"
              name="maximum"
              value={memoryOptions.maximum}
              onChange={handleMemoryOptionsChange}
              min="1"
            />
          </div>
        ...
        <p>
          <small>Note: 1 page = 64KB. Changes will take effect after reloading.</small>
        </p>
      </div>
...
export default SharedMemoryDarkSwapDemo;