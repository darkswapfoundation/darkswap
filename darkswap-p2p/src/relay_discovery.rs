...
impl libp2p::kad::store::RecordStore for MemoryStore {
    ...
    fn get(&self, _key: &Key) -> Result<Option<libp2p::kad::record::Record>, Self::Error> {
        Ok(None)
    }
    
    fn put(&mut self, _record: libp2p::kad::record::Record) -> Result<(), Self::Error> {
        Ok(())
    }
    
    fn remove(&mut self, _key: &Key) -> Result<(), Self::Error> {
        Ok(())
    }
    
    fn iter(&self) -> Box<dyn Iterator<Item = libp2p::kad::record::Record> + '_> {
